...
# -----------------------------------------------------------------------------
# G E T   M O D E L   A N D   V A R I A N T
#
# Parses setxkbmap arguments and echoes the model and variant (if supplied)
# -----------------------------------------------------------------------------
GetModelAndVariant()
{
    while [ $# -gt 1 ]; do
        if [ "$1" = "-model" ] || [ "$1" = "-variant" ]; then
            echo " $1 $2"
            shift
        fi
        shift
...
kbgen()
{
    file=$(printf "../instfiles/km-%08s.ini" "$1" | tr ' ' '0')
    desc="$2"
    os="$3"
    shift 3
    # Extract the model and any variant from the setxkbmap arguments
    # in "$@" and add then to the description
    mv=$(GetModelAndVariant "$@")
    desc="$desc$mv"
    setxkbmap "$@"
    ./xrdp-genkeymap -c "Operating system: $os" -c "$desc" "$file"
}
