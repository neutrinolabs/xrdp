...
rail_get_window_data(Window window)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_get_window_data:"));
    rv = 0;
...
rail_send_init(void)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_send_init:"));
    make_stream(s);
    init_stream(s, 8182);
...
rail_init(void)
{
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_init:"));
    xcommon_init();
...
rail_startup(void)
{
    ...
    {
        LOGM((LOG_LEVEL_INFO, "rail_init: found RandR extension"));
        st = XRRQueryVersion(g_display, &ver_maj, &ver_min);
        if (st)
        {
            LOGM((LOG_LEVEL_INFO, "rail_init: RandR version major %d minor %d", ver_maj, ver_min));
        }
...
rail_process_exec(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_INFO, "chansrv::rail_process_exec:"));
    ...
    LOGM((LOG_LEVEL_DEBUG, "  flags 0x%8.8x ExeOrFileLength %d WorkingDirLength %d "
    ...
    if (g_strlen(ExeOrFile) > 0)
    {
        ...
        LOGM((LOG_LEVEL_DEBUG, "rail_process_exec: pre"));
        ...
        LOGM((LOG_LEVEL_DEBUG, "rail_process_exec: post"));
    }
...
rail_win_popdown(void)
{
    ...
    for (i = nchild - 1; i >= 0; i--)
    {
        ...
        {
            LOGM((LOG_LEVEL_DEBUG, "  dismiss pop up 0x%8.8lx", children[i]));
...
rail_close_window(int window_id)
{
    ...
    LOGM((LOG_LEVEL_INFO, "chansrv::rail_close_window:"));
...
{
    LOGM((LOG_LEVEL_DEBUG, "my_timeout: g_got_focus %d", g_got_focus));
    if (g_focus_counter == (int)(long)data)
    {
        LOGM((LOG_LEVEL_DEBUG, "my_timeout: g_focus_counter %d", g_focus_counter));
        rail_win_popdown();
    }
}
...
rail_process_activate(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_activate:"));
    ...
    if (index < 0)
    {
        LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_activate: window 0x%8.8x not in list",
                 window_id));
        return 0;
    }
    ...
    g_focus_counter++;
    g_got_focus = enabled;
    LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x enabled %d", window_id, enabled));
    ...
    if (enabled)
    {
        ...
        else
        {
            ...
            LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_activate: calling XRaiseWindow 0x%8.8x", window_id));
            XRaiseWindow(g_display, window_id);
            LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_activate: calling XSetInputFocus 0x%8.8x", window_id));
        ...
        LOGM((LOG_LEVEL_DEBUG, "  window attributes: override_redirect %d",
...
rail_process_system_param(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_system_param:"));
    in_uint32_le(s, system_param);
    LOGM((LOG_LEVEL_DEBUG, "  system_param 0x%8.8x", system_param));
    /*
    ...
    {
        LOGM((LOG_LEVEL_DEBUG, "  restore rail windows"));
                                 ...
                                 unsigned char** data, unsigned long* count)
{
    ...
    if ((ret != Success || nitems < 1) && atom_return == None)
    {
        ...
        LOGM((LOG_LEVEL_DEBUG, "  rail_get_property %s: failed", prop_name));
        XFree(prop_name);
        return 1;
    }
...
rail_win_get_state(Window win)
{
    ...
    if (data && nitems > 0)
    {
        ...
        XFree(data);
        LOGM((LOG_LEVEL_DEBUG, "  rail_win_get_state: %d", rv));
    }
...
rail_win_set_state(Window win, unsigned long state)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "  rail_win_set_state: %ld", state));
    ...
    if (old_state == -1)
    {
        ...
        LOGM((LOG_LEVEL_DEBUG, "  rail_win_set_state: create WM_STATE property"));
    }
    else
    {
...
{
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_minmax_window 0x%8.8x:", window_id));
    if (max)
    {
...
rail_restore_window(int window_id)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_restore_window 0x%8.8x:", window_id));
    XGetWindowAttributes(g_display, window_id, &window_attributes);
    if (window_attributes.map_state != IsViewable)
    {
        XMapWindow(g_display, window_id);
...
rail_process_system_command(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_system_command:"));
    in_uint32_le(s, window_id);
    ...
    if (index < 0)
    {
        LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_system_command: window 0x%8.8x not in list",
                 window_id));
        return 0;
    }
    ...
    {
        case SC_SIZE:
            LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x SC_SIZE", window_id));
            break;
        case SC_MOVE:
            LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x SC_MOVE", window_id));
            break;
        case SC_MINIMIZE:
            LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x SC_MINIMIZE", window_id));
            rail_minmax_window(window_id, 0);
            break;
        case SC_MAXIMIZE:
            LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x SC_MAXIMIZE", window_id));
            break;
        case SC_CLOSE:
            LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x SC_CLOSE", window_id));
            rail_close_window(window_id);
            break;
        case SC_KEYMENU:
            LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x SC_KEYMENU", window_id));
            break;
        case SC_RESTORE:
            LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x SC_RESTORE", window_id));
            rail_restore_window(window_id);
            break;
        case SC_DEFAULT:
            LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x SC_DEFAULT", window_id));
            break;
        default:
            LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x unknown command command %d",
                     window_id, command));
            break;
    }
...
rail_process_handshake(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_handshake:"));
    in_uint32_le(s, build_number);
    LOGM((LOG_LEVEL_DEBUG, "  build_number 0x%8.8x", build_number));
    return 0;
}
...
rail_process_notify_event(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_notify_event:"));
    in_uint32_le(s, window_id);
    in_uint32_le(s, notify_id);
    in_uint32_le(s, message);
    LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x notify_id 0x%8.8x message 0x%8.8x",
...
rail_process_window_move(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_window_move:"));
    in_uint32_le(s, window_id);
    in_uint16_le(s, si16);
    left = si16;
    in_uint16_le(s, si16);
    ...
    in_uint16_le(s, si16);
    bottom = si16;
    LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x left %d top %d right %d bottom %d width %d height %d",
...
rail_process_local_move_size(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_local_move_size:"));
    in_uint32_le(s, window_id);
    ...
    in_uint16_le(s, si16);
    pos_y = si16;
    LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x is_move_size_start %d move_size_type %d "
...
{
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_min_max_info:"));
    return 0;
}
...
rail_process_client_status(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_client_status:"));
    in_uint32_le(s, flags);
    LOGM((LOG_LEVEL_DEBUG, "  flags 0x%8.8x", flags));
    return 0;
}
...
rail_process_sys_menu(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_sys_menu:"));
    in_uint32_le(s, window_id);
    in_uint16_le(s, si16);
    left = si16;
    in_uint16_le(s, si16);
    top = si16;
    LOGM((LOG_LEVEL_DEBUG, "  window_id 0x%8.8x left %d top %d", window_id, left, top));
    return 0;
}
...
rail_process_lang_bar_info(struct stream *s, int size)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_lang_bar_info:"));
    in_uint32_le(s, language_bar_status);
    LOGM((LOG_LEVEL_DEBUG, "  language_bar_status 0x%8.8x", language_bar_status));
    return 0;
}
...
{
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_appid_req:"));
    return 0;
}
...
{
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_appid_resp:"));
    return 0;
}
...
{
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_process_exec_result:"));
    return 0;
}
             ...
             int total_length)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_data_in:"));
    in_uint8(s, code);
    in_uint8s(s, 1);
    in_uint16_le(s, size);
    ...
    switch (code)
    {
        ...
        case TS_RAIL_ORDER_EXEC_RESULT: /* 128 */
            ...
            break;
        default:
            LOGM((LOG_LEVEL_DEBUG, "rail_data_in: unknown code %d size %d", code, size));
            break;
    }
...
rail_win_send_text(Window win)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_win_send_text:"));
    ...
    if (rwd != 0)
    {
        if (data != 0)
        {
            if (rwd->valid & RWD_TITLE)
            {
                ...
                {
                    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_win_send_text: skipping, title not changed"));
                    g_free(data);
                    XFree(rwd);
                    return 0;
                }
    ...
    {
        LOGM((LOG_LEVEL_ERROR, "chansrv::rail_win_send_text: error rail_get_window_data_safe failed"));
        g_free(data);
        return 1;
    }
    if (data && len > 0)
    {
        LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_win_send_text: 0x%8.8lx text %s length %d",
                 win, data, len));
        make_stream(s);
...
rail_destroy_window(Window window_id)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_destroy_window 0x%8.8lx", window_id));
    make_stream(s);
    init_stream(s, 1024);
...
rail_show_window(Window window_id, int show_state)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_show_window 0x%8.8lx 0x%x", window_id, show_state));
    make_stream(s);
    init_stream(s, 1024);
...
rail_create_window(Window window_id, Window owner_id)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_create_window 0x%8.8lx", window_id));

    rwd = rail_get_window_data_safe(window_id);
    if (rwd == 0)
    {
        LOGM((LOG_LEVEL_ERROR, "chansrv::rail_create_window: error rail_get_window_data_safe failed"));
        return 0;
    }
    ...
    LOGM((LOG_LEVEL_DEBUG, "  x %d y %d width %d height %d border_width %d", x, y, width,
             height, border));

    index = list_index_of(g_window_list, window_id);
    if (index == -1)
    {
        LOGM((LOG_LEVEL_DEBUG, "  create new window"));
        flags = WINDOW_ORDER_TYPE_WINDOW | WINDOW_ORDER_STATE_NEW;
        list_add_item(g_window_list, window_id);
    }
    else
    {
        LOGM((LOG_LEVEL_DEBUG, "  update existing window"));
    ...
    LOGM((LOG_LEVEL_DEBUG, "  title %s", title_bytes));
    ...
    LOGM((LOG_LEVEL_DEBUG, "  set title info %d", title_size));
...
rail_configure_request_window(XConfigureRequestEvent* config)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_configure_request_window: mask %d", mask));
    if (mask & CWStackMode)
    {
        LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_configure_request_window: CWStackMode "
                 "detail 0x%8.8x above 0x%8.8lx", config->detail, config->above));
        if (config->detail == Above)
        {
            LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_configure_request_window: bring to front "
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_configure_request_window: 0x%8.8x", window_id));


    LOGM((LOG_LEVEL_DEBUG, "  x %d y %d width %d height %d border_width %d", config->x,
    ...
    if (index == -1)
    {
        ...
        LOGM((LOG_LEVEL_ERROR, "chansrv::rail_configure_request_window: window not mapped"));
        return 0;
    }
...
rail_configure_window(XConfigureEvent *config)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_configure_window 0x%8.8x", window_id));
...
{
    LOGM((LOG_LEVEL_INFO, "rail_desktop_resize:"));
    return 0;
}
...
rail_xevent(void *xevent)
{
    ...
    LOGM((LOG_LEVEL_DEBUG, "chansrv::rail_xevent:"));
    ...
    switch (lxevent->type)
    {
        case PropertyNotify:
            ...
            LOGM((LOG_LEVEL_DEBUG, "  got PropertyNotify window_id 0x%8.8lx %s state new %d",
        ...
        case ConfigureRequest:
            LOGM((LOG_LEVEL_DEBUG, "  got ConfigureRequest window_id 0x%8.8lx", lxevent->xconfigurerequest.window));
        ...
        case CreateNotify:
            LOGM((LOG_LEVEL_DEBUG, " got CreateNotify window 0x%8.8lx parent 0x%8.8lx",
                     lxevent->xcreatewindow.window, lxevent->xcreatewindow.parent));
            rail_select_input(lxevent->xcreatewindow.window);
            break;

        case DestroyNotify:
            LOGM((LOG_LEVEL_DEBUG, "  got DestroyNotify window 0x%8.8lx event 0x%8.8lx",
        ...
        case MapRequest:
            LOGM((LOG_LEVEL_DEBUG, "  got MapRequest window 0x%8.8lx", lxevent->xmaprequest.window));
            XMapWindow(g_display, lxevent->xmaprequest.window);
            break;

        case MapNotify:
            LOGM((LOG_LEVEL_DEBUG, "  got MapNotify window 0x%8.8lx event 0x%8.8lx",
        ...
        case UnmapNotify:
            LOGM((LOG_LEVEL_DEBUG, "  got UnmapNotify 0x%8.8lx", lxevent->xunmap.event));
            ...
            if (is_window_valid_child_of_root(lxevent->xunmap.window))
            {
                ...
                LOGM((LOG_LEVEL_DEBUG, "  window 0x%8.8lx is unmapped", lxevent->xunmap.window));
                if (index >= 0)
                {
        ...
        case ConfigureNotify:
            LOGM((LOG_LEVEL_DEBUG, "  got ConfigureNotify 0x%8.8lx event 0x%8.8lx", lxevent->xconfigure.window,
        ...
        case FocusIn:
            LOGM((LOG_LEVEL_DEBUG, "  got FocusIn"));
            g_focus_win = lxevent->xfocus.window;
            break;

        case FocusOut:
            LOGM((LOG_LEVEL_DEBUG, "  got FocusOut"));
            break;

        case ButtonPress:
            LOGM((LOG_LEVEL_DEBUG, "  got ButtonPress"));
            break;

        case EnterNotify:
            LOGM((LOG_LEVEL_DEBUG, "  got EnterNotify"));
            break;

        case LeaveNotify:
            LOGM((LOG_LEVEL_DEBUG, "  got LeaveNotify"));
            break;

        case ReparentNotify:
            LOGM((LOG_LEVEL_DEBUG, "  got ReparentNotify window 0x%8.8lx parent 0x%8.8lx "
