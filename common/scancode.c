...
#if !defined(USE_EVDEV_KEYCODES) && !defined(USE_BASE_KEYCODES)
#error "Don't know which keycode table to user on this platform"
#endif
...
#elif defined (USE_BASE_KEYCODES)
// Sources:-
...
#else
#  error "Can't determine which scancode to keycode map to use"
#endif
...
{
    unsigned int min = 0;
    unsigned int max = ELEMENTS(scancode_to_keycode_map);
    unsigned short rv = 0;

    // Check scancode is in range of map
    if (scancode >= scancode_to_keycode_map[min].scancode &&
            scancode <= scancode_to_keycode_map[max - 1].scancode)
    {
        ...
        while (1)
        {
            ...
            if (scancode == scancode_to_keycode_map[index].scancode)
            {
                rv = scancode_to_keycode_map[index].keycode;
                break;
            }
...
{
    unsigned short rv = 0;
    if (*iter < ELEMENTS(scancode_to_keycode_map))
    {
        rv = scancode_to_keycode_map[*iter].scancode;
        ++(*iter);
    }
